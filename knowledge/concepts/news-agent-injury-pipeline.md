...
- [[daily/lcash/2026-05-01.md]] - **Stage 2 systemic failure root cause found**: Windows `.CMD` wrapper truncates multi-line `-p` args at first newline — Stage 2 prompt was silently truncated; fix: single-line `-p` arg with full instructions in workspace files. **Production milestone**: 12 events, 33 picks in 8 hours — highest sustained rate. Four fixes confirmed working in production: tier-c training-knowledge, lineup-PASS-unconditionally, prediction-markets-as-soft (6 soft books for NBA), dedup retry-after-window. Stage 2 optimized: serial→parallel (asyncio.create_task), timeout 600→900s (Conley pick loss), odds.md→odds.jsonl (33% faster, $0.60/event). Prediction market books whitelisted in pick_writer.py. Sonnet hallucinated file writes (McDaniels test: agent described writing picks but file was empty). Odds movement persistence: JSONB column on news_agent_events; snapshot key bug (missing `line`) caused 130 bogus movements; Discord enriched with per-pick odds + book name + top 5 movers (Sessions 08:18, 08:53, 09:58, 13:01, 15:37)
