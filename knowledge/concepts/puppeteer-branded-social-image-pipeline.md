...
- **66 concepts (31 original + 35 new content hooks) / 90+ PNGs** across 3 sizes: 1080×1080 (feed), 1080×1350 (stories), 1200×628 (LinkedIn); 34 hooks mirrored to `nano-ads.js` for AI photo pipeline
...
- [[daily/lcash/2026-06-27.md]] - 6 content images with real logo overlay confirmed complete in Downloads; Brand Copy Guide PDF generated as snapshot from copy-bank.md; copy bank §1/§5 drift reconfirmed — 2 of 6 built images came from §5 avatar hooks not §1 ad headlines, needs §9 "promote winners up" reconciliation pass; organic captions needed for the 6 images (Session 14:13). Gallery UI enhanced with download buttons (grid hover, modal single-size, "download all 3 sizes"); `<button>` → `div role="button"` for valid nested `<a>` tags; Gowun Batang serif font tested as sub-header option (Session 20:28). 35 new content hooks plugged into `concepts.js` as `statement` type across 7 themes (Presence, No Bot, Status, Free, Undetectable, Memory, Solopreneur Sam); 34 mirrored into `nano-ads.js` as `HOOK_ADS` (nb-02 nested-quotes excluded from AI pipeline — unreliable text rendering); "Photos" section added to gallery UI with `/api/assets` endpoint and `AssetGrid` component with lightbox; live filesystem scan API pattern (no manifest) for AI-generated assets; gallery now runs on port 5181 (Session 22:19)
