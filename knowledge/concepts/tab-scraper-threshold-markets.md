...
### Eve (VPS) Deployment (2026-05-13)

On 2026-05-13, the TAB scraper was deployed to Eve (VPS) as part of the native AU scraper rollout. Direct connections from Eve to the TAB competition endpoint work without proxies (Webshare proxies returned 407 from IP-allowlist mode, but this is irrelevant). VPS showed 100% freshness for TAB data after deployment, confirming the `curl_cffi`-based REST architecture works reliably from the VPS environment.
...
- [[daily/lcash/2026-05-13.md]] - Eve deployment: direct connections work without proxies; VPS 100% freshness; deployed alongside Betr and TabTouch in AU scraper rollout (Session 14:41)
