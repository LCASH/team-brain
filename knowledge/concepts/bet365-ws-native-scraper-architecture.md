...
### I99 Zero-Click Architecture and Parser Fix (Session 16:30)

On 2026-05-07, the NBA scraper was validated with a **zero in-game clicks architecture**: one CDP-trusted sidebar click for discovery, then `page.goto(game_url + '/I99/')` for each game. No tab clicks, no in-page interactions beyond the single discovery click. This was driven by bet365's isTrusted detection (see [[concepts/bet365-istrusted-synthetic-click-detection]]) — all click-based flows were redesigned to use `page.goto()` exclusively.

The I99 BB wizard delivers **286-482 PAs per game** in a single fetch — all SGM-eligible player props across 13 prop types and 10-18 players per game. This was verified locally via AdsPower with 1,174 ODs (odds) across 3 of 4 NBA games (average 391/game).

A critical parser bug was discovered and fixed: **MG-without-NA reset bug**. When a Market Group (MG) record lacked an `NA=` field, the parser's `cur_mg` variable was not being reset, causing all subsequent PA records to be silenced (associated with the wrong/missing MG context). This single fix unlocked the entire I99 pipeline from 0% yield to full yield. The "Player " prefix in I99 wizard PAs (`NA=Player Anthony Edwards`) also required a fallback: the parser tries both `"Player " + name` and raw name when matching against the EV name map.

Additionally, **NST (X-Net-Sync-Term) direct fetch was confirmed dead**: even with a fresh 4-second-old NST token, `fetch()` initiated from CDP `Runtime.evaluate` returns 200/0 bytes. bet365 distinguishes between SPA-initiated HTTP requests and CDP-initiated programmatic fetches. Navigation-fired coupon captures via Playwright `page.on("response")` remain the only proven data extraction path.

### 4-Sport WS Injection Verdict (Session 20:56)

A comprehensive WS injection test across 4 sports (NBA, MLB, NRL, Tennis) definitively confirmed that **HTTP polling is the only viable primary data source** for pre-game props. The server acknowledges 100% of injected WS subscriptions but delivers 0% U-frame deltas for any of them — even for an actively-trading live Tennis match. See [[concepts/bet365-ws-subscription-injection-viability]] for the full 4-sport test methodology and EMPTY ack semantics analysis.

This verdict confirms the hybrid architecture: `_refresh_loop` (10-15s HTTP wizard refresh) as the primary data source, with WS listener as a passive supplement that captures whatever bet365 happens to feature on its global `OV_POPULAR` firehose.

### Horse Race PM Delta Confirmation (Session 23:32) — Partial Verdict Revision

Session 23:32 partially revised the Session 20:56 verdict: a **live horse race probe** (FI=194196581) captured real PM-format U-deltas with `DO=`, `OD=`, and `OH=` (odds history — last 3 prices) in every push. The prior 4-sport test's 0% delta delivery was because all test fixtures were pre-game with static lines — EMPTY means "buffer dormant, no price changes" not "streaming unsupported."

The revised architecture for v4:

| Market State | Primary Source | Secondary Source |
|-------------|---------------|-----------------|
| **Pre-game (stable)** | HTTP polling (30s NBA, 60s MLB) | WS listener (OV_POPULAR firehose) |
| **Near-tipoff / Live** | PM subscriptions (sub-second) | HTTP polling (safety net) |

PM subscriptions become the primary odds source once markets become volatile (near tipoff or live). HTTP polling is demoted to a safety net for live markets but remains primary for pre-game. This represents a significant latency improvement for near-tipoff markets where the 30-60s HTTP cycle was the remaining bottleneck. See [[concepts/bet365-ws-pm-live-delta-confirmation]] for the complete horse race probe analysis and delta payload format.
...
- [[concepts/bet365-ws-pm-live-delta-confirmation]] - Horse race probe confirmed PM deltas work for volatile markets; partially revises the Session 20:56 "HTTP only" verdict for near-tipoff/live markets
...
- [[daily/lcash/2026-05-07.md]] - **WS pre-game coverage gap confirmed**: `OV_POPULAR_30_0` is a cross-sport global firehose, NOT per-game feeds; 0 PA_ID overlap across 275+ frames on NBA/MLB/NRL; pre-game lines are static (zero WS deltas hours before tipoff); May 6 "703 updates/120s" was in-play trading, not generalizable; hybrid architecture decided: add `_refresh_loop` (10-15s HTTP wizard refresh) as primary, WS as passive supplement near tipoff; sequential `monitor.goto()` cancels previous game's subscriptions; CDPSession on orchestrator page blocks Playwright discovery — fix: dedicated fresh page for discovery (Sessions 09:18, 18:40, 19:20). **Disjoint PA_ID spaces**: HTTP wizard returns static catalog IDs (1224xxx) while per-G-ID page.goto and WS both operate in live trading space (1238-1240xxx) — explains MLB G-ID walk's WS overlap advantage over NBA I99 wizard; per-G-ID navigation fires both HTTP partial AND activates SPA WS subscription simultaneously (Session 19:51). **I99 zero-click architecture validated**: 286-482 PAs/game in single wizard fetch; MG-without-NA parser bug was root cause of 0% yield — single fix unlocked entire pipeline; zero in-game clicks architecture (one CDP sidebar click + page.goto); NST direct fetch confirmed dead (200/0bytes even with 4s-old token); 1,174 ODs across 3 NBA games locally (Session 16:30). **4-sport WS injection death verdict**: 100% subscribe ack, 0% delta delivery across NBA/MLB/NRL/Tennis including live match; EMPTY ack ≠ data will follow; HTTP polling confirmed as only viable primary source (Session 20:56). **Horse race PM delta confirmation** (Session 23:32): live horse race FI=194196581 captured real PM U-deltas with DO=, OD=, OH= (odds history) fields; EMPTY reinterpreted as "buffer dormant" not "refused"; v4 architecture revised: PM primary for volatile/live markets, HTTP demoted to safety net; pre-game HTTP remains primary (30s NBA, 60s MLB)
