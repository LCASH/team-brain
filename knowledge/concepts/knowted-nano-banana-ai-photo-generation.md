...
### ICP-Tested Hero Line Validation and Content-Mode Pivot (Session 22:55)

In the final creative session on 2026-06-16, lcash ran ICP testing on 8 candidate hero lines across all four avatar segments, producing a clear verdict: 3 lines were universal winners, 1 was retired, and the rest were segment-specific. The core testing rule: **call/meeting/bot language is invisible to students; lecture/class language is invisible to professionals.** Lines that use "meetings" exclude the Student Sophie persona; lines that reference "lectures" exclude the Closer Chloe persona. Only lines framed around the universal pain ("notes" + "listening") work across all segments.

A significant strategic pivot occurred: **content-mode over ad-mode** for this batch. Instead of producing CTA-heavy ad cards ("Join the waitlist"), the output shifted to hook-plus-wordmark posts designed to feel native in a social feed. The caption carries any CTA, not the image itself. This reflects the content-first GTM principle: build audience interest through relatable content before asking for conversion.

**Logo reference wiring**: The real Knowted logo (brain-and-checkmark icon from Google Drive's brand folder) was wired into nano banana's generator as a reference image. Nano banana reproduces reference logos *recognizably* but not pixel-perfectly — acceptable for social feed content where the mark provides brand association, not print-quality reproduction. Pixel-perfect logo placement still uses the HTML template pipeline.

**Hero line evolution**: "Listen, or take notes. Not both." was rejected as too much of a riddle — it requires re-reading to decode, which disqualifies it as a headline (must decode instantly). The replacement: **"Just listen. Knowted takes the notes."** — direct, no decoding required. The longer "Listen, or take notes" phrasing was retained as an organic post opener (where the reader's mode is exploratory, not scanning) but dropped from the headline tier.

**Variant-pick workflow validated**: Generating 2 nano banana variants per line and keeping the cleaner one (discarding text glitches) is now the standard production workflow. This accepts AI text unreliability as a given and treats variant generation as QA, not a sign of tool failure.

### Hero Line Final Iteration (Session 23:08)
...
- [[concepts/knowted-content-first-gtm-strategy]] - The content-first GTM strategy that the social media posting cadence serves; content-mode pivot reflects "build audience before asking for conversion"
- [[concepts/knowted-icp-voc-avatar-derivation]] - The four ICP avatars that the 4-week posting plan targets with one lane per avatar; ICP testing determined which hero lines are universal vs segment-specific
- [[concepts/knowted-waitlist-website-iteration]] - Hero line testing results feed back into website copy decisions; "Just listen. Knowted takes the notes." emerged from the same testing that validated the website's three pillars
