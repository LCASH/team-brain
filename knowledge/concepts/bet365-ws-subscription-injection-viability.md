...
aliases: [ws-injection, ws-subscribe-probe, bs-topic-prefix, interceptor-on-spa-ws, ws-auth-replay, disjoint-pa-id-spaces, ws-injection-dead, 4-sport-injection-test, empty-ack-semantics, mg-id-namespace-mismatch, fi-routing-key, pm-vs-l-format, subscribe-vs-delivery-format]
...
### FI Routing Key and Subscribe vs Delivery Format Distinction (Session 22:27)

A final deep decode session on 2026-05-07 (Session 22:27) clarified the WS topic routing architecture. The `X` in both subscribe topics (`PM{X}-{PA_ID}`) and delivery topics (`L{X}-{PA_ID}_30_0`) is a **Fixture Instance ID (FI)** — a 9-digit value from the event_id space, NOT a Market Group ID as previously labeled in the MG_ID section above. The 9-digit numbers found in the wizard body (194110689 etc.) are indeed FI values, not per-MG identifiers.

This also clarifies the distinction between **subscribe format** and **delivery format**:

| Direction | Format | Purpose |
|-----------|--------|---------|
| Client → Server (subscribe) | `PM{FI}-{PA_ID}` | Request price snapshot for a specific participant |
| Server → Client (delivery) | `L{FI}-{PA_ID}_30_0` | Push delta update when odds change |

These are fundamentally different frame types. `PM` subscribes are **snapshot requests** that only succeed if the server has buffered state for that participant. Pre-game fixtures have no buffered state (no trading activity), so PM subscribes return EMPTY — not because the subscription was rejected, but because there is literally no snapshot to deliver. This extends the Session 20:56 finding: even a **currently-live fixture with actively-pushing PAs** returns EMPTY on explicit PM subscribe.

`L`-format deltas are **broadcast-only** — they flow from broadcast topics (`OVInPlay_30_0`, `OV_POPULAR_30_0`) that the server independently decides to push to. No per-PA subscription triggers L-format delivery. The server's broadcast engine operates on its own featured-content algorithm, completely decoupled from client subscription requests.

The subscribe envelope format was also confirmed: `\x16\x00` + topics_csv + `,A_{1626-byte-rotating-token}` + `\x01`. Frame type bytes: `\x14` = snapshot/EMPTY ack, `\x15` = U updates/D datasets.

Per-sport HTTP refresh timings were finalized: 30s for NBA (wizard response is larger but fewer games), 60s for MLB (more games, lighter per-game response). These are the sole pre-game data source — `ws_decoded=0` on pre-game fixtures is expected behavior, not a bug.
...
### Untested Hypotheses (Session 22:57) — Partially Resolved (Session 23:32)

Two hypotheses were identified at the end of Session 22:57:

1. **Time-of-day gating**: WS streams may activate 30-60 minutes before tipoff. **Partially validated**: Session 23:32's horse race probe confirmed PM deltas flow for volatile/live markets. A near-tipoff sports test (30-60 min before first pitch) remains needed to confirm the activation window for sports props specifically.

2. **Betslip-triggered stream allocation**: The server may only allocate WS delta buffers for betslip-tracked PAs. **Disproved**: Session 23:32 confirmed PM-topic deltas flow for page-rendered PAs without betslip interaction. The BS topic is a parallel channel, not a prerequisite for PM streaming. Betslip-added PAs stream via BOTH PM and BS channels simultaneously.

### Horse Race Live Delta Confirmation (Session 23:32)

Session 23:32 provided the breakthrough that recontextualized the entire WS investigation: a **live horse race probe** (FI=194196581) captured real PM-format U-deltas with `DO=` (decimal odds), `OD=` (fractional), and `OH=` (last 3 price snapshots — line movement history baked into every push). The MLB pre-game betslip probe returned EMPTY for all 7 PAs, confirming the pre-game dormancy pattern.

The critical reinterpretation: **EMPTY means "buffer dormant, nothing has changed"** — NOT "subscribe refused" or "streaming unsupported." This partially reverses the Session 20:56 architecture decision: for volatile/live markets, PM subscriptions should be the primary data source with HTTP polling as safety net. Pre-game stable lines still require HTTP polling as primary. See [[concepts/bet365-ws-pm-live-delta-confirmation]] for the complete analysis.
...
- [[concepts/bet365-ws-pm-live-delta-confirmation]] - Session 23:32 horse race probe that resolved the untested hypotheses: PM deltas DO work for volatile markets, EMPTY = dormant buffer not refusal

## Sources

- [[daily/lcash/2026-05-07.md]] - Disjoint PA_ID spaces: HTTP wizard 1224xxx (static) vs WS 1238-1240xxx (live trading); page.goto(/G{gid}/S^1/) fires HTTP partial AND activates WS subscription — captures in live trading range; betslip validates via HTTP re-fetch at "Place Bet" not WS; BS prefix for betslip topics; 2 WS connections confirmed (premws 56 frames, pshudws 1 frame) (Session 19:51). Interceptor-on-SPA-WS confirmed working (131KB page body, both WS open, no blanking); 3-topic injection probe: stable PA → EMPTY, OVInPlay → F-frame, bogus → EMPTY; standalone auth replay returns 400 (session-bound); auth token reusable within session (same token 3 frames/60s) not cross-session; SPA subscribes only at UI action moment; consolidated to `project_bet365_ws_protocol_reversed.md`; April 15 "injection fails" was standalone-specific, not interceptor (Session 20:23). **4-sport injection death verdict**: comprehensive test across NBA, MLB, NRL, Tennis — 100% ack rate, 0% delta delivery across all sports; Tennis live match (Giron vs Cilic) 84/84 EMPTY acks in 60s; EMPTY ack = "registered" not "data will follow"; 3 PA_ID namespaces confirmed (HTTP catalog 1219-1224xxx, WS live-trading 1238-1240xxx, betslip 194xxx); architecture pivot to HTTP polling (10-15s) as primary with WS as passive supplement; April 15 render-state authorization confirmed correct — server decides delivery independently of client subscription (Session 20:56). **MG_ID namespace mismatch**: HTTP wizard body contains only 3-digit local MG IDs (340-351) and 5-6 digit global prop-template IDs (181378, 181551), NOT the 9-digit per-instance MG_IDs required for WS `L{MG_ID}-{PA_ID}_30_0` topics; only 5 event-adjacent 9-digit numbers in wizard body; May 6 `matchbettingcontentapi/coupon` MG_ID finding not validated across 4 NBA HTTP endpoints; dual namespace problem (MG_ID + PA_ID both disjoint) makes WS topic construction from HTTP data doubly impossible; 3 forward paths identified: live-fixture probe, OV_POPULAR capture, event-adjacent ID testing (Session 21:27). **FI routing key + subscribe vs delivery format**: `X` in `PM{X}-{PA_ID}` and `L{X}-{PA_ID}_30_0` is Fixture Instance ID (FI), not MG_ID — corrects Session 21:27 nomenclature; PM subscribes are snapshot requests (no buffered state for pre-game → EMPTY); L-format deltas are broadcast-only (no per-PA subscription triggers them); even currently-live fixtures return EMPTY on explicit PM subscribe; subscribe envelope: `\x16\x00` + topics_csv + `,A_{1626b-token}` + `\x01`; frame types `\x14`=snapshot/EMPTY, `\x15`=U/D; per-sport HTTP refresh finalized: 30s NBA, 60s MLB; `ws_decoded=0` on pre-game is expected (Session 22:27). **Untested hypotheses** (Session 22:57): time-of-day gating (WS may activate 30-60 min before tipoff), betslip-triggered stream allocation (server may only push deltas for betslip-tracked PAs); user's tennis observation (markets moved with betslip open) may support hypothesis 2; Stage 7 JSON deliverable `/tmp/bet365_props_TEX_NYY.json` (288 props, 12 types, 19 players); full protocol decode in `brain/findings/2026-05-07-bet365-ws-deep-decode.md`. **Horse race live delta confirmation** (Session 23:32): betslip probe on MLB pre-game returned EMPTY for all 7 PAs (confirmed dormancy); horse race FI=194196581 captured real PM U-deltas with DO=, OD=, OH= fields; EMPTY reinterpreted as "buffer dormant" not "refused"; same PA streams via PM and BS simultaneously; v4 architecture revised: PM primary for volatile markets, HTTP safety net; hypothesis 1 (time-gating) partially validated, hypothesis 2 (betslip-triggered) disproved
