...
### Untested Hypotheses (Session 22:57)

Two hypotheses remain untested as of 2026-05-07 and are deferred to a morning MLB first-pitch validation:

1. **Time-of-day gating**: WS streams may activate 30-60 minutes before tipoff, when bet365 transitions games from "pre-game" to "approaching live" status. All prior testing was hours before tipoff when lines are genuinely static. A probe during the last 30 minutes before first pitch could reveal time-bound WS activation.

2. **Betslip-triggered stream allocation**: The server may only allocate WS delta buffers for PAs that a user has added to their betslip. The `BS` topic prefix discovered in Session 19:51 suggests betslip interactions create a separate, prioritized data channel. If the server only pushes pre-game deltas for betslip-tracked PAs, this would explain why broadcast topics (`OV_POPULAR`) carry deltas for in-play markets (which all users see) but not for arbitrary pre-game PAs (which require user action to "activate").

The user's observation — "markets moved while I was watching with betslip open" during a live Tennis session — may be the missing clue for hypothesis 2. However, this could also be explained by the live match generating genuine odds movement visible on the `OVInPlay` broadcast, independent of betslip state.
